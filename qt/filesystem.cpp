...
bool FileSystemIoHandler::writableCDrive() const
{
    auto drive = mPaths.value('C');
    return !drive.path.isEmpty() && drive.writable;
}
    ...
    if (!driveStruct.path.isEmpty()) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            if (dir.exists(component)) {
                ...
                QString componentLower = component.toLower();
                for (const QString& f : dir.entryList()) {
                    if (f.toLower() == componentLower) {
                        foundEntry = f;
                        break;
                    }
                }
            ...
            if (i == n - 1) {
                ...
                dir = QDir(dir.filePath(component));
                // and continue
            }
        }
    }
