...
void MainWindow::onFileOpened(const QString& path)
{
    ...
    if (extension == "sis") {
        ...
        if (!drive.isEmpty()) {
            ...
            getRuntime().setDrive(Drive::D, getSharedDrive());
            QString dirPrefix = QString("C:\\System\\Apps\\") + info.dir().dirName() + "\\";
            QIcon icon = getRuntime().iconForAif(dirPrefix + info.completeBaseName() + ".aif");
            if (!icon.isNull()) {
                setWindowIcon(icon);
            }
            getRuntime().run(dirPrefix + info.fileName());
        } else {
