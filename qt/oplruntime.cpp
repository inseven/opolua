    ...
    , mCursorTimer(nullptr)
{
    ...
    lua_getfield(L, 1, "searchers"); // 2: orig searchers
    lua_createtable(L, 1, 0); // 3: newsearchers
    lua_rawgeti(L, 2, 1); // package.searchers[1]
    lua_rawseti(L, 3, 1); // newsearchers[1] = package.searchers[1]

    lua_pushcfunction(L, searcher);
    lua_rawseti(L, 3, 2);

    lua_setfield(L, 1, "searchers");
...
void OplRuntime::restart()
{
    ...
    if (running()) {
        mShouldRestart = true;
        interrupt();
        // will result in restart() being re-called from onThreadExited
    } else {
        lua_settop(L, 0);
        require(L, "runtime");
...
bool OplRuntime::writeableCDrive() const
{
    return mFs->writeableCDrive();
}
...
void OplRuntime::run(const QDir& cDrive, const QString& devicePath)
{
    ...
    pushIohandler();
    emit startedRunning(mFs->getNativePath(devicePath));
    startThread();
}
...
void OplRuntime::runInstaller(const QString& file, const QString& dest)
{
    mFs->addMapping('C', dest, true);
...
void OplRuntime::threadFn(OplRuntime* self)
{
    ...
    // 2: function to call
    // 3-top: arguments
    self->mRet = lua_pcall(L, lua_gettop(L) - 2, 0, 1);
...
int OplRuntime::asyncRequest(lua_State* L)
{
    ...
    if (requestName == "getevent") {
        ...
        Q_ASSERT(mPendingEventRequestType == none);
        mPendingEventRequestType = getevent;
        mPendingEventRequestRef = requestHandle;
    } else if (requestName == "keya") {
        QMutexLocker lock(&mMutex);
        Q_ASSERT(mPendingEventRequestType == none);
        mPendingEventRequestType = keya;
        mPendingEventRequestRef = requestHandle;
...
bool OplRuntime::completeAnyRequest_locked(lua_State *L)
{
    ...
    if (mPendingEventRequestType == getevent && mEvents.count()) {
        auto event = mEvents.takeFirst();
        int ref = mPendingEventRequestRef;
        mPendingEventRequestRef = LUA_NOREF;
        mPendingEventRequestType = none;
        mMutex.unlock();
        writeEventCompletion(L, ref, event, getevent);
        doCompleteRequest(L, KErrNone);
        return true;
    } else if (mPendingEventRequestType == keya) {
        while (mEvents.count()) {
            auto event = mEvents.takeFirst();
            if (event.isKeyEvent()) {
                int ref = mPendingEventRequestRef;
                ...
                mPendingEventRequestType = none;
                mMutex.unlock();
                writeEventCompletion(L, ref, event, keya);
                doCompleteRequest(L, KErrNone);
                return true;
            } else {
