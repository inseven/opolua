...
int OplRuntime::cancelRequest(lua_State* L)
{
    return call([this, L]() {
        ...
        if (h) {
            ...
            mPendingEventRequestType = AsyncEventRequestType::cancelled;
        }
        mMutex.unlock();
        return 0;
    });
...
// Public fn, call from main thread (signals Lua thread)
...
// Private fn, callable from any thread (does not signal)
...
bool OplRuntime::completeAnyRequest_locked(lua_State *L)
{
    ...
    if (mPendingEventRequestType == AsyncEventRequestType::cancelled) {
        mPendingEventRequestRef = LUA_NOREF;
        mPendingEventRequestType = AsyncEventRequestType::none;
        mMutex.unlock();
        doCompleteRequest(L, KErrIOCancelled);
        return true;
    } else if (mPendingEventRequestType == AsyncEventRequestType::getevent && mEvents.count()) {
...
// Must only be called from completeAnyRequest_locked
